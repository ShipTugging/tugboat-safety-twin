...
시연 중 반복 조작은 3D 장면 아래 **선수 기준 상세 설정** 도크를 사용한다. 선수·선미·좌현·우현, CCTV 시점, 조향각, 예인줄, 속력, RPM을 드래그 없이 숫자 입력 또는 단축값 버튼으로 바꿀 수 있으며 Python 서버 상태와 최근 결과도 같은 행에서 확인한다. [도크 설계·검증 기록](docs/2026-09-15-bow-control-dock.md)
