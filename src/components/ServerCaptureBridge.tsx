...
export function ServerCaptureBridge({params,telemetry,analysis}:{params:SimulationParams;telemetry:TelemetryState;analysis:ServerAnalysis}){
 ...
 useFrame(()=>{
  ...
  try{
   ...
   const cameraContext=JSON.stringify({mount:'TUG_SAG_CAM',position:p.towPosition??'ahead',fov:p.cameraFov??60,jitter:p.cameraJitter,rotation:p.cameraRotationJitter});
