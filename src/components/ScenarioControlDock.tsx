...
const CAMERA_LABELS: Record<SimulationParams['cameraMode'], string> = {
  TUG_SAG_CAM: 'CCTV · 줄 감시', TUG_AFT_DECK: 'CCTV · 선미 덱', TUG_BRIDGE: 'CCTV · 조타실 80°',
  orbit: '자유 시점', tugChase: '예인선 추적', bridgeView: '선교 시점', topDown: '상공 시점', cinematic: '시네마틱',
};
const RISK_LABELS: Record<string, string> = { Normal: '정상', Loaded: '하중 상태', GirtingRisk: '거팅 위험', Developing: '위험 진행', Critical: '임계 위험', UNKNOWN: '관측 불가' };
...
export function ScenarioControlDock({ params, onChangeParams, onReset, onTriggerQuickRelease, analysis }: { params: SimulationParams; onChangeParams: (next: Partial<SimulationParams>) => void; onReset: () => void; onTriggerQuickRelease: () => void; analysis: ServerAnalysis }) {
  ...
  return <section className="scenario-control-dock" aria-label="선수 기준 상세 설정">
    ...
    <div className="scenario-control-grid">
      ...
      <NumericControl control="tugSteeringAngle" label="조향각" value={params.tugSteeringAngle} quick={[-85, -68, 0, 68, 85]} disabled={locked} onChange={value => setNumber('tugSteeringAngle', value)} />
      ...
      <NumericControl control="shipSpeed" label="본선 속력" value={params.shipSpeed} quick={[0, 6, 10, 14]} disabled={locked} onChange={value => setNumber('shipSpeed', value)} />
