import type { SimulationParams, TimeOfDay, TowPosition } from '../types/maritime';
...
export function randomizeEnvironment(random:()=>number=Math.random,index=Math.floor(random()*600)):SimulationParams {
  ...
  const towPosition=(['astern','port','starboard','ahead'] as TowPosition[])[Math.floor(index/3)%4];
  return {
    ...
    cameraMode,timeOfDay,towPosition,quickReleaseActive:false,soundEnabled:false,
