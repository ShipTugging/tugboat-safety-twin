...
> 2026-09-16 이후 V2 계약은 [Risk V2 문서](2026-09-16-risk-v2.md)를 우선한다. 아래는 V1 도입 기록을 포함한다. V2는 세션별 시간 이력과 품질 검사를 사용하며, 미검출 시에도 IMU 위험 상태가 반환될 수 있다.
...
- 서버 주소 검사, null/비정상 숫자·상태 거부, 실제 모드에서 정답 힌트 제외 등 포함 91개 테스트 통과. TypeScript·Vite 빌드 통과.
...
- 최신 `server/models/best.pt`를 실제로 로드해 선수 예인 영상의 10프레임 간격 60장을 `/analyze`에 전송했다. 전 프레임에서 towline 검출, confidence 0.960~0.971, 중앙 왕복 193.2ms, 비전 값 null 0건이었다. 흰색 무관 영상은 200 응답과 `risk_state=UNKNOWN`, `fusion_mode=null`을 반환했다.
...
미검출 응답의 `fusion_mode=null`이 화면에 그대로 표시되던 부분을 `관측 불가`로 바꿨다. 이는 정상 판정이 아니라 비전 관측 실패 상태다.
